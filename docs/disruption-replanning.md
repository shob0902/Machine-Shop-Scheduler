...
The official assignment says this is the heart of the assignment. This
document explains the algorithm in `backend/scheduler/replanner.py` and
`backend/services/disruption_service.py`.
...
Freeze in-progress work       (start_bucket <= now_bucket < end_bucket),
                               ...
                               just got delayed) - Section 25 "preserve
...
Apply the disruption as a constraint Overlay:
    ...
    rework             -> inject a new task into the routing (Section 14)
    power_cut          -> block every machine for that (day, shift)
                           ...
                           as a cost line item instead (Section 15)
    |
...
schedule" (Section 25 explicitly forbids that) - it is the same schedule
...
Price the difference (cost_delta = new_cost_breakdown - old_cost_breakdown)
    |
Generate the data-driven "owner action" recommendation (Section 45)
```

## Why not just regenerate from scratch?

Because Section 25 says not to, and because it would be operationally
...
## What the comparison shows (Section 26)
...
## The official-style demo scenario (Section 27)
...
(Section 7, enforced by a test — see `tests/test_data_generation.py::
test_only_three_operators_qualified_for_grinding`), this scenario is
...
## The owner action recommendation (Section 45)
...
## Power cut / generator (Section 15)
...
operations at `GENERATOR_COST_MULTIPLIER` (3x, Section 15) instead of the
